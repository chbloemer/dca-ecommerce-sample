package dev.domaincentric.sample.ecommerce.product.adapter.outgoing.inventory;
 ...
 *
 * <p>This adapter implements Product's {@link ProductStockDataPort} by delegating to the Inventory
 * context's {@link InventoryService} OHS.
 *
 * <p>This adapter is the ONLY place in Product context that imports from the Inventory context,
...
public class InventoryStockDataAdapter implements ProductStockDataPort {
  ...
  public Optional<StockData> getStockData(final ProductId productId) {
    return inventoryService.getStock(productId).map(this::toStockData);
  }
  ...
  public Map<ProductId, StockData> getStockData(final Collection<ProductId> productIds) {
    return inventoryService.getStock(productIds).entrySet().stream()
        .collect(Collectors.toMap(Map.Entry::getKey, entry -> toStockData(entry.getValue())));
  }

  private StockData toStockData(final StockInfo stockInfo) {
    return new StockData(
        stockInfo.productId(), stockInfo.availableStock(), stockInfo.isAvailable());
  }
}
