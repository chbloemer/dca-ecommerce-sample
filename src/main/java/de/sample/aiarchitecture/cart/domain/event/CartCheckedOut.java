 ...
 *
 * <p><b>Integration Event:</b> This event crosses bounded context boundaries and represents a
 * public contract between the Cart context and consuming contexts (e.g., Product, Order). Published
 * by Cart context when checkout completes successfully.
 *
 * <p><b>Consumers:</b>
 *
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Cross-Context Integration Pattern:</b> This event uses lightweight DTOs (ItemInfo) instead
 * of full domain objects to avoid bounded context violations. The DTOs contain only the data needed
 * for other contexts to react. Consumers should use an Anti-Corruption Layer to translate this
 * event into their own ubiquitous language.
 *
 ...
 *
 * <p><b>DDD Pattern:</b> Events should be self-contained and not expose domain objects from one
 * bounded context to another. This ensures proper context isolation.
 */
public record CartCheckedOut(
  ...
  public static CartCheckedOut now(
    ...
    return new CartCheckedOut(
        UUID.randomUUID(), cartId, customerId, totalAmount, itemCount, itemInfos, Instant.now(), 1);
  }
